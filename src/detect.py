...
def create_trainer_engine(model_enc, optim_enc, model_dec, optim_dec, xfrm_sampler, mixup_sampler,
    ...
    def _update(engine, batch):
        ...
        # Regeneration loss
        loss_recon = - torch.mean(torch.sum(Normal(x_pred, 1.0).log_prob(x), dim=[1, 2, 3]))
        # Mixup loss
        x_mix = model_dec.mix(x, x.roll(mixup_shift, dims=0), lambda_)
        loss_mixup = - torch.mean(torch.sum(Normal(x_mix_pred, 1.0).log_prob(x_mix), dim=[1, 2, 3]))
        ...
        #   Geo transform loss
        v_pert = xfrm_sampler(v_pred)
        v_xfrm = xfrm_pose(v_pred, v_pert)
        x_geo_xfrm = geo_xfrm(x, d, v_pert)
        x_model_xfrm = model_dec(v_xfrm)
        x_model_xfrm = x_model_xfrm.where(x_geo_xfrm > 0, torch.tensor([0.], device=x_geo_xfrm.device))
        loss_pert = - torch.mean(torch.sum(Normal(x_model_xfrm, 1.0).log_prob(x_geo_xfrm), dim=[1, 2, 3]))

        ###
        # Decoder loss
        loss_dec = loss_recon + loss_mixup + loss_pert
        # loss_dec = loss_recon + loss_mixup
        # Encoder loss
        loss_enc = loss_recon + loss_mixup + loss_v_rel + loss_pert
